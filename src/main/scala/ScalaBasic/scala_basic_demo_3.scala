...
object scala_basic_demo_3{
    ...
    def main(args: Array[String]){
        ...
        for (x <- shipList) {println(x)}

        // 6) apply a function literal to a list! map() can be used to apply any function to every item in a collection.
        val backwardShips = shipList.map( (x: String) => {x.reverse})
        for (x <- backwardShips) {println(x)} 

        val backwardShips2 = shipList.map( (x: String) => {x + " -- "})
        for (x <- backwardShips2) {println(x)} 
